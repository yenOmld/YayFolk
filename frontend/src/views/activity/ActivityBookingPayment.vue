<template>
  <div class="payment-page">
    ...
    <div v-if="loading" class="state-card">Loading payment information...</div>
    <div v-else-if="!booking" class="state-card">This booking is unavailable for payment.</div>

    <div v-else class="payment-layout">
      <section class="main-column">
        <article class="panel-card hero-panel">
          <p class="eyebrow">Activity Checkout</p>
          <h1>{{ success ? 'Payment Completed' : 'Confirm Activity Payment' }}</h1>
          <p v-if="success">
            Payment is complete. You can review the booking detail or open the check-in QR code now.
          </p>
          <p v-else>
            This project currently uses a simulated in-app payment flow so the booking, payment, and check-in journey can run end to end.
          </p>
        </article>
        ...
        <article class="panel-card order-panel">
          <div class="order-head">
            <div>
              <h2>{{ booking.activityTitle || 'Activity Booking' }}</h2>
          ...
          <div class="summary-grid">
            ...
            <div><span>Participant</span><strong>{{ booking.participantName || '-' }}</strong></div>
            <div><span>Phone</span><strong>{{ booking.participantPhone || '-' }}</strong></div>
            <div><span>Activity Time</span><strong>{{ formatRange(booking.startTime, booking.endTime) }}</strong></div>
            <div class="full-span"><span>Location</span><strong>{{ fullLocation }}</strong></div>
          </div>
        </article>
        ...
        <article v-if="!success && booking.canPay" class="panel-card">
          ...
          <div class="hint-box">
            ...
            <p>The selected method is stored on the booking, and payment is completed immediately in-app for this merge stage.</p>
          </div>
        </article>

        <article v-else class="panel-card success-panel">
          <div class="success-icon"><i class="bx bx-check-circle"></i></div>
          <h2>{{ success ? 'Payment successful and QR is ready' : 'This booking no longer needs payment' }}</h2>
          <p>{{ successMessage }}</p>
        </article>
      </section>
      ...
      <aside class="side-column">
        <article class="panel-card sticky-card">
          ...
          <div class="summary-list">
            <p>Booking Status: {{ displayStatus.label }}</p>
          ...
          <div class="action-list">
            ...
            <button class="secondary-btn" @click="openDetail">Open Order Detail</button>
...
const paymentMethods = [
  { value: 'simulated', label: 'Simulated Pay', desc: 'Immediate in-app completion for the merged booking flow.', icon: 'bx bx-credit-card', theme: 'default' },
  { value: 'alipay', label: 'Alipay', desc: 'Stored as the chosen method while payment still completes in-app for now.', icon: 'bx bxl-alipay', theme: 'alipay' }
]
...
const paymentStatusText = computed(() => ({ paid: 'Paid', unpaid: 'Unpaid', refunded: 'Refunded' }[booking.value?.paymentStatus] || booking.value?.paymentStatus || 'Unpaid'))
const paymentLabel = computed(() => ({ pending: 'Pending', simulated: 'Simulated Pay', free: 'Free', alipay: 'Alipay' }[booking.value?.paymentType || selectedMethod.value] || 'Pending'))
const successMessage = computed(() => booking.value?.paymentStatus === 'paid'
  ? 'The booking is now marked as paid. You can open the QR code and continue to on-site check-in.'
  : 'This booking does not require another payment.')
