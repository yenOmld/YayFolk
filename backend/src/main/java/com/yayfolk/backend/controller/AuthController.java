...
public ResponseDto register(@Valid @RequestBody RegisterRequest request) {
    try {
        ...
        // 注册用户
            com.yayfolk.backend.entity.User user = userService.register(username, request.getPassword(), request.getNickname(), phone, email, request.getCountry(), request.getLangCode(), request.getRegionCode());
    ...
    public ResponseDto login(@Valid @RequestBody LoginRequest request, HttpServletRequest httpRequest) {
        try {
            com.yayfolk.backend.entity.User user = userService.login(request.getUsername(), request.getPassword());
    ...
    public ResponseDto loginByCode(@RequestBody LoginByCodeRequest request, HttpServletRequest httpRequest) {
        try {
            ...
            com.yayfolk.backend.entity.User user = userService.findByUsername(username);
    ...
    public RedirectView githubCallback(@RequestParam("code") String code, @RequestParam("state") String state, HttpServletRequest httpRequest) {
        try {
            ...
            com.yayfolk.backend.entity.User user = userService.createOrUpdateUserFromGithub(githubId, username, email, name, avatarUrl);
